...
@dag(
    dag_id="Lumon_Customer_Data_Processing_Daily",
...
def pipeline_function():
    ...
    if current_env == "Prod":
        json_config_path = default_args["config_path"] + \
            "/Lumon_Customer_Data_Processing_Daily_prod.json"
    else:
        json_config_path = default_args["config_path"] + \
            "/Lumon_Customer_Data_Processing_Daily.json"

    cfg = get_json_config(json_config_path)
    region_name = cfg["aws_reg_rs"]
    global_param_config = get_ssm_parameters(
        "a360_dev_global_param_config", region_name, False)
        ...
        if not pg_result:
            ...
            for data in pg_result:
                ...
                unload_redshift_datashare_data(
                    bucket_name, filename, src_tab, incremental_field, feed, cfg, redshift_iam_role, secrets_config)
    ...
    def stg_a360_salarmycontacttest(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "stg_a360_salarmycontacttest",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("SKip")
        ...
        if "SALARMYCONTACTTEST" in pulled_list:
            file_present_ind = True
        else:
            file_present_ind = False
        src_cd = "SALARMYCONTACTTEST"
        run_a360_stg_job(
            cfg=cfg,
            source_cd=src_cd,
    ...
    def archive_incoming_files(**kwargs):
        ...
        for file in file_archive_list:
            ...
            feed_type = list_config[source_code]["feed_type"]
            move_and_delete_s3_file(
                bucket_name,
    ...
    def extract_salarmycontacttest(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "extract_salarmycontacttest",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("Skip")
        ...
        v_cur_date = ti.xcom_pull(
            task_ids="source_file_stats", key="batch_timestamp")
        task_name = "extract_salarmycontacttest"
        run_extract_task(
            cfg=cfg,
            task_name=task_name,
            default_args=default_args,
