...
from a360.python.mz_task import *
...
@dag(
    dag_id="lumon_validation_automation",
...
def lumon_validation_automation():
    ...
    a360_global_params = get_ssm_parameters("a360_global_params", region_name, False)
    a360_global_params = json.loads(a360_global_params)
    webapp_secret_name = cfg["a360_weapp_secret"]
    a360_webapp_secret_config = get_secret(webapp_secret_name, region_name)
    ...
    def send_failure_email(context, data_details):
        ...
        notification_payload = {
            ...
            "STATUS": "ERROR",
            "OPERATION": "ENDED",
            "PAYLOAD": {"DATA_INGESTION_ID": data_details["data_ingestion_id"]},
        }

        api_response = a360_api_call(
            method="POST",
            secret_config=a360_webapp_secret_config,
            a360_global_params=a360_global_params,
            json=notification_payload,
            client=cfg["company_id"],
        )
        logging.info(
            f"API Response of Notification for ERROR {json.dumps(api_response,indent=2)}"
        )
    ...
    def athenaRejectCount(table_name, filename):
        ...
        query = f"""WITH data AS (SELECT 
                                    ...
                                    FROM 
                                        "{athena_validation_database}".stg_{table_name}_reject 
                                        --where replace(lower(filename_dax_{table_name}),'.gz','') = replace(lower('{filename}'),'.gz','')
                                )
                                SELECT
    ...
    def fun_check_feed_enabled(**kwargs):
        ti = kwargs["ti"]
        data_details = ti.xcom_pull(
            task_ids="get_data_ingestion_details", key="data_details"
        )
        if data_details["is_enabled"]:
            logging.info("Feed is enabled. Proceeding with validation.")
            return True
        else:
            logging.info("Feed is disabled. Skipping validation.")
            return False
    ...
    def get_data_ingestion_details(**kwargs):
        ...
        try:
            ...
            notification_payload = {
                ...
                "STATUS": "DEFAULT",
                "OPERATION": "STARTED",
                "PAYLOAD": {"DATA_INGESTION_ID": data_ingestion_id},
            }
            ...
            logging.info(
                f"API Response of Notification for STARTED {json.dumps(api_response,indent=2)}"
            )

            sql = """
                SELECT c.company_name,a.frequency,a.data_ingestion_id, a.data_source_type, data_source_object, uploaded_file_name, feed, a.company_id, content_type, feed_type, config, priority_processing, b.cloud_region,is_enabled,a.incremental_field,a.datashare_name
    ...
    def check_feed_enabled(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            ...
            # send_email()
            raise AirflowSkipException
        val = fun_check_feed_enabled(**kwargs)
        return val
    ...
    def check_feed_disabled(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "check_feed_disabled",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            raise AirflowSkipException
        val = not (fun_check_feed_enabled(**kwargs))
        return val
    ...
    def unload_data_from_client_redshift(**kwargs):
        ...
        unload_redshift_datashare_data(
            ...
            incremental_field=data_details['incremental_field'],  
            feed=data_details['feed'],          
            cfg=cfg,
            redshift_iam_role=redshift_iam_role,
            secrets_config=secrets_config,
            datashare_name=data_details["datashare_name"],
        )
    ...
    def copy_ingestion_config(**kwargs):
        ...
        if region_name == s3_bucket_region:
            ...
            if data_details["priority_processing"]:
                copy_s3_object(
                    copy_source_bucket=s3_bucket_name,
                    copy_source_path=f"{s3_bucket_path}{data_details['data_ingestion_id']}_priority_config.json",
                    copy_target_bucket=bucket_name,
                    copy_target_path=f"config/pyspark_json/{data_details['data_ingestion_id']}_priority_config.json",
                )
        else:
            copy_cross_region_s3_datasync(
                s3_bucket_name,
            ...
            if data_details["priority_processing"]:
                copy_cross_region_s3_datasync(
                    ...
                    s3_bucket_region,
                    region_name,
                    specific_file=f"{data_details['data_ingestion_id']}_priority_config.json",
                )
        ...
        if data_details["priority_processing"]:
            move_and_delete_s3_file(
                bucket_name,
                f"config/pyspark_json/{data_details['data_ingestion_id']}_priority_config.json",
                bucket_name,
                f"config/pyspark_json/dax_{data_details['feed'].lower()}_priority.json",
            )
    ...
    def generate_code_and_run_stg_load(**kwargs):
        ...
        if data_details["content_type"].lower() == "full" and data_details[
            ...
            crc_prev_tablename = f"stg_{data_details['feed'].lower()}_crc_prev"
            crc_pii_prev_tablename = f"stg_{data_details['feed'].lower()}_crc_pii_prev"
            input_file = f"DAX_{data_details['feed'].upper()}_*,{crc_prev_tablename},{crc_pii_prev_tablename}"
        ...
        if data_details["content_type"].lower() == "incremental" or data_details[
            "feed_type"
        ].lower() in ("campaign data", "lookup data"):
            # 2. Define input_files
            input_file = f"DAX_{data_details['feed'].upper()}_*"
            # 3. Define format file
            format_file = (
                f"{format_file_path_glue}dax_{data_details['feed'].lower()}.json"
            )
    ...
    def generate_code_and_run_ods_load(**kwargs):
        ...
        v_src_s3_path = f"s3://{bucket_name}/process_fcvrl/athena_validation_glue/STG_{data_details['feed']}/"
        ### Call Redshift procedure to configure and Validate ODS Load ###
        procedure_call = "CALL a360_load.prc_configure_ods_dwh ('{}','{}','{}','{}', '{}','{}', '{}','{}')".format(
            data_details["feed"],
    ...
    def data_pipeline_dag_generation(**kwargs):
        ...
        param_env = task_instance.xcom_pull(
            task_ids="get_data_ingestion_details",key="env"
        )
        ...
        company_id = data_details.get("company_id", "")
        # Modify to handle when frequency is None
        frequency = (data_details.get("frequency") or "").capitalize()
        ...
        dag_file_name = (
            f"{company_name}_{data_category}{f'_{frequency}' if frequency else ''}.py"
        )
    ...
    def generate_metadata_for_orchestration(**kwargs):
        ...
        company_id = data_details.get("company_id", "")
        pg_result, col_names, data_category = fetch_dag_gen_parameters(
            feed_type, frequency, cfg, pg_secrets_config, company_id
        )

        json_template = {
            "key": "<source_cd>",
            "file_pattern": "DAX_<source_code>_*",
            "header": True,
            "fullfile": "<full_file_flag>",
            "reject_mailboxes": [],
            "write_to_parquet": False,
            "frequency": "<frequency>",
            "extract_mailboxes": ["LUMON1RT"],
            "extract_mft_delay": 90,
        }
        ...
        for recs in pg_result:
            ...
            feed_json["file_pattern"] = feed_json["file_pattern"].replace(
                "<source_code>", source_cd.upper()
            )
            # list_config_inputs.append(json.dumps(feed_json, indent=4, cls=CustomEncoder))
            list_config_inputs.append(feed_json)

        request_mapping_key = f"{data_category}_{frequency}" if frequency else data_category
        logging.info(f"request_mapping_key: {request_mapping_key}")
        cfg["request_type"] = cfg["request_type_mapping"].get(request_mapping_key, "UNKNOWN")
        logging.info(f"cfg['request_type']: {cfg['request_type']}")
        config = create_dag_config(
            cfg, bucket_name, sec_name, list_config_inputs, frequency
        )
        ...
        config_file_name = (
            f"{company}_{data_category}{f'_{frequency}' if frequency else ''}.json"
        )
        file_type_mapping = task_instance.xcom_pull(
            task_ids="data_pipeline_dag_generation_1", key="file_type_mapping"
        )
        if file_type_mapping is None:
            file_type_mapping = task_instance.xcom_pull(
                task_ids="data_pipeline_dag_generation_2", key="file_type_mapping"
            )
        ...
        if data_details.get("priority_processing", ""):
            ...
            pg_result, col_names, data_category = fetch_dag_gen_parameters(
                feed_type, frequency, cfg, pg_secrets_config, company_id, priority_flag
            )
            list_config_inputs = []
            feed_name_list = []
            json_template = {
            ...
            request_mapping_key = f"{data_category}"
            logging.info(f"request_mapping_key: {request_mapping_key}")
            cfg["request_type"] = 5
            logging.info(f"cfg['request_type']: {cfg['request_type']}")
    ...
    def dag_config_git_integration(**kwargs):
        ...
        file_type_mapping = task_instance.xcom_pull(
            task_ids="generate_metadata_for_orchestration_1", key="file_type_mapping"
        )
        if file_type_mapping is None:
            file_type_mapping = task_instance.xcom_pull(
                task_ids="generate_metadata_for_orchestration_2",
                key="file_type_mapping",
            )
    ...
    def deploy_to_production(**kwargs):
        ...
        src_s3_path = (
            f"s3://{bucket_name}/process_fcvrl/athena_glue/STG_{data_details['feed']}/"
        )

        ### Call Redshift procedure to Deploy ODS table, view, DWH view, ODS load, Metadata tables ###
        procedure_call = (
            "CALL a360_load.prc_deploy_ods_dwh ('{}','{}','{}','{}','{}','{}','{}')".format(
                data_details["feed"],
                data_details["feed_type"],
                dedupe_fields_str,
                data_details["content_type"],
                src_s3_path,
                data_details['incremental_field'],
                cfg["spectrum_schema"]
            )
        )
        print(procedure_call)
        ...
        # Archive file after validation success
        file_name = (
        ...
        s3_a360.move_and_delete_s3_file(
            bucket_name,
            f"incoming/{file_name}",
            bucket_name,
            f"archive/validation/{file_name}",
        )
    ...
    def update_validation_status(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
        ...
        logging.info(
            f"API Response of Notification for SUCCESS {json.dumps(api_response,indent=2)}"
        )

    data_details = get_data_ingestion_details()
    check_feed_enabled_task = check_feed_enabled()
    check_feed_disabled_task = check_feed_disabled()
    data_pipeline_dag_generation_1 = data_pipeline_dag_generation.override(
        task_id="data_pipeline_dag_generation_1"
    )()
    data_pipeline_dag_generation_2 = data_pipeline_dag_generation.override(
        task_id="data_pipeline_dag_generation_2"
    )()
    generate_metadata_for_orchestration_1 = (
        generate_metadata_for_orchestration.override(
            task_id="generate_metadata_for_orchestration_1"
        )()
    )
    generate_metadata_for_orchestration_2 = (
        generate_metadata_for_orchestration.override(
            task_id="generate_metadata_for_orchestration_2"
        )()
    )
    dag_config_git_integration_1 = dag_config_git_integration.override(
        task_id="dag_config_git_integration_1"
    )()
    dag_config_git_integration_2 = dag_config_git_integration.override(
        task_id="dag_config_git_integration_2"
    )()
    update_validation_status_1 = update_validation_status.override(
        task_id="update_validation_status_1"
    )()
    update_validation_status_2 = update_validation_status.override(
        task_id="update_validation_status_2"
    )()
    ...
    # Connect the tasks
    (data_details >> [check_feed_enabled_task, check_feed_disabled_task])

    (
        check_feed_enabled_task
        >> branch
        >> [
            unload_data_from_client_redshift(),
            copy_file_from_client_bucket(),
        ...
        >> generate_code_and_run_stg_load()
        >> generate_code_and_run_ods_load()
        >> data_pipeline_dag_generation_1
        >> generate_metadata_for_orchestration_1
        >> dag_config_git_integration_1
        >> deploy_to_production()
        >> update_validation_status_1
    )

    (
        check_feed_disabled_task
        >> data_pipeline_dag_generation_2
        >> generate_metadata_for_orchestration_2
        >> dag_config_git_integration_2
        >> update_validation_status_2
    )
