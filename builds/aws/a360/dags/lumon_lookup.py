...
from a360.python.a360_mz_task import run_ods_job as run_a360_ods_job
from a360.python.a360_mz_task import business_to_internal_ingestion_integration
...
default_args = {
    ...
    "retries": 0,
    "email_to": "Mayur.Dubey@data-axle.com",
...
@dag(
    ...
    description="Lumon Lookup Data Pipeline",
...
def pipeline_function():
    ...
    def business_to_internal_s3_file_transfer(**kwargs):
        ...
        destination_bucket = cfg["bucket_name"]
    ...
    def source_file_stats(**kwargs):
        ...
        if env == "dev":
            ...
            get_file_record_count(
                batch_id=v_batchid,
                glue_job_name="glue_source_file_stats_lumon",
                incoming_path=folder_prefix,
                redshift_config_path=default_args["redshift_config_paths"].replace(
                    "{}", bucket_name
                ),
                s3_bucket=bucket,
                table_source_file_stats="stg_source_file_stats_lookup",
            ...
            procedure_call = "CALL a360_core.prc_check_dup_files_lookup ({},'{}')".format(
    ...
    def ods_a360_modelscores(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "ods_a360_modelscores",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("SKip")
        ...
        if src_cd in pulled_list:
            run_a360_ods_job(
                gv_checkpoint_code="PRC_ODS_MODELSCORES",
                cfg=cfg,
                source_cd=src_cd,
                bucket_name=bucket_name,
                secrets_config=secrets_config,
                batch_id=v_batchid,
                default_args=default_args,
                v_cur_date=v_cur_date,
                v_cur_user=default_args["ods_user"],
            )
        else:
            print(
                "ODS Load not required. No {} file found in current batch".format(
                    src_cd
                )
            )
    ...
    def ods_a360_statelookup(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "ods_a360_modelscores",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
        ...
        sql_string = """COPY a360_ods.a360_statelookup
                    FROM 's3://axle-a360-demo-3185/incoming/DAX_StateLookup_20241024_121212.csv'
                    IAM_ROLE 'arn:aws:iam::082126963185:role/da-mz-redshift-role'
                    FILLRECORD
                    ACCEPTINVCHARS
                    IGNOREHEADER 1
                    MAXERROR AS 10000
                    ...
                    DELIMITER ','"""
        print(sql_string)
        response = redshift_query_execute(
                    sql_string, cfg, secrets_config=secrets_config,query_type=""
                )
