...
@dag(
    dag_id="Lumon_Transaction_Data_Processing_Weekly",
...
def pipeline_function():
    ...
    if current_env == "Prod":
        json_config_path = default_args["config_path"] + \
            "/Lumon_Transaction_Data_Processing_Weekly_prod.json"
    else:
        json_config_path = default_args["config_path"] + \
            "/Lumon_Transaction_Data_Processing_Weekly.json"
    ...
    def stg_a360_testdata11(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "stg_a360_testdata11",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
    ...
    def dps_extract_mcd_incr_demo(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            ...
            print("SKip")
            raise AirflowSkipException
        del_obj = s3_aet.del_all_object_from_s3_folder(
    ...
    def load_individual_dim(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "load_individual_dim",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
        ...
        procedure_call = f"""CALL a360_load.prc_a360_dim_indiv_load(
        {v_execid},
