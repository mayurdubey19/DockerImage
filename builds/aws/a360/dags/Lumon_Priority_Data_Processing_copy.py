...
@dag(
    dag_id="Lumon_Priority_Data_Processing_copy",
...
def pipeline_function():
    ...
    if current_env == "Prod":
        json_config_path = default_args["config_path"] + \
            "/Lumon_Priority_Data_Processing_prod.json"
    else:
        json_config_path = default_args["config_path"] + \
            "/Lumon_Priority_Data_Processing.json"
    ...
    athena_database = get_ssm_parameters(
        "a360_global_params", region_name, False)
    ...
    def dps_mcd_hist_bkp(**kwargs):
        ...
        s3_key = f"{cfg['s3_key_athena_glue']}{cfg['dp_config']['daily']['athena_partitioned_table']}/"
    ...
    def prepare_incoming_file_from_db(**kwargs):
        ...
        query_type = "query"
        feed_types = ['Customer Data', 'Prospect Data', 'Suppression Data']
        sql = (f"select * from mz360.data_ingestion where feed_type IN {tuple(feed_types)} and is_enabled = 'True' and "
        ...
        if not pg_result:
            ...
            for data in pg_result:
                ...
                unload_redshift_datashare_data(
                    bucket_name, filename, src_tab, cfg, redshift_iam_role, secrets_config)
    ...
    def source_file_stats(**kwargs):
        ...
        if env == "dev":
            ...
            # in case of empty df it is overwriting stg_source_file_stats_priority making file_record_count col to varchar
            if not df.empty:
                df.to_sql(
                    "stg_source_file_stats_priority",
                    con=engine,
                    if_exists="replace",
                    index=False,
                    schema="a360_stg",
                )
            # Get file_record_count for files with S3 select failure, capture in tmp table mzb_aet_stg.stg_source_file_stats_priority_tmp
            ...
            get_file_record_count(
                ...
                s3_bucket=bucket,
                table_source_file_stats="stg_source_file_stats_priority",
            ...
            procedure_call = "CALL a360_core.prc_check_dup_files_priority ({},'{}')".format(
    ...
    def business_to_internal_s3_file_transfer(**kwargs):
        ...
        print("pg_secrets_config: ", str(pg_secrets_config))
        logging.info("business to internal s3 file transfer Process")
        feed_types = ['Eofissue', 'MODELSCORES', 'MEMBERTESTNEW', 'EXTOPTOUTSUP',
                      'SALARMYCONTACT', 'DONORACCOUNTS220', 'LUMONPROSPECT', 'SALARMYACCOUNT']
        business_to_internal_ingestion_integration(
            cfg, secrets_config, pg_secrets_config, bucket_name, feed_types
        )
    ...
    def stg_a360_membertestnew(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "stg_a360_membertestnew",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("SKip")
            raise AirflowSkipException
        src_cd = "MEMBERTESTNEW"
        file_present_ind = True
        run_a360_stg_job(
            cfg=cfg,
            source_cd=src_cd,
    ...
    def stg_a360_extoptoutsup(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "stg_a360_extoptoutsup",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("SKip")
    ...
    def stg_a360_salarmycontact(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "stg_a360_salarmycontact",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("SKip")
            raise AirflowSkipException
        src_cd = "SALARMYCONTACT"
        file_present_ind = True
        run_a360_stg_job(
            cfg=cfg,
            source_cd=src_cd,
    ...
    def stg_a360_donoraccounts220(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "stg_a360_donoraccounts220",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("SKip")
            raise AirflowSkipException
        src_cd = "DONORACCOUNTS220"
        file_present_ind = True
        run_a360_stg_job(
            cfg=cfg,
            source_cd=src_cd,
    ...
    def stg_a360_lumonprospect(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "stg_a360_lumonprospect",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("SKip")
            raise AirflowSkipException
        src_cd = "LUMONPROSPECT"
        file_present_ind = True
        run_a360_stg_job(
            cfg=cfg,
            source_cd=src_cd,
    ...
    def stg_a360_salarmyaccount(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "stg_a360_salarmyaccount",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("SKip")
            raise AirflowSkipException
        src_cd = "SALARMYACCOUNT"
        file_present_ind = True
        run_a360_stg_job(
            cfg=cfg,
            source_cd=src_cd,
    ...
    def process_prematch_data(**kwargs):
        ...
        procedure_call = f"""CALL a360_stg.prc_a360_proc_caller({gv_batchexecid},'{gv_checkpoint}','{p_cur_date}','{p_cur_user}','{p_process_name}');"""
        print(procedure_call)
        redshift_query_execute(
            sql=procedure_call,
            cfg=cfg,
    ...
    def load_transactional_tables(**kwargs):
        ...
        if any(category in kwargs["dag"].dag_id.lower() for category in ("campaignandlookup")):
            proc_param = f",'{redshift_iam_role}' )"
        else:
            proc_param = " ) "
