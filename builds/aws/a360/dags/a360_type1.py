...
default_args = {
    ...
    "retries": 0,
    "email_to": "Mitesh.Shivramwar@data-axle.com",
...
@dag(
    ...
    description="Lumon Daily Data Pipeline",
...
def pipeline_function():
    ...
    def business_to_internal_s3_file_transfer(**kwargs):
        ...
        business_to_internal_ingestion_integration(
            cfg, secrets_config, pg_secrets_config, bucket_name
        )
    ...
    def prepare_incoming_file_from_db(**kwargs):
        ...
        for data in pg_result:
            ...
            # src_tab = "ds_a360demo.mzb_aet_stg.email_response_a72"
        ...
        logging.info("Redshift data unload successful")
    ...
    def stg_a360_daconsumer(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "stg_a360_daconsumer",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
