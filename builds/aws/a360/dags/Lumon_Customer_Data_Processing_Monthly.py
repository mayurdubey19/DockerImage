...
@dag(
    dag_id="Lumon_Customer_Data_Processing_Monthly",
...
def pipeline_function():
    ...
    if current_env == "Prod":
        json_config_path = default_args["config_path"] + \
            "/Lumon_Customer_Data_Processing_Monthly_prod.json"
    else:
        json_config_path = default_args["config_path"] + \
            "/Lumon_Customer_Data_Processing_Monthly.json"
    ...
    def dps_mcd_hist_bkp(**kwargs):
        ...
        ##### Daily  Backups ######
        step_set = "mcd_hist_bkup_monthly"
        ...
        s3_key = f"{cfg['s3_key_athena_glue']}{cfg['dp_config']['monthly']['athena_partitioned_table']}/"
    ...
    def stg_a360_qacustomermonthly(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "stg_a360_qacustomermonthly",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("SKip")
        ...
        if "QACUSTOMERMONTHLY" in pulled_list:
            file_present_ind = True
        else:
            file_present_ind = False
        src_cd = "QACUSTOMERMONTHLY"
        run_a360_stg_job(
            cfg=cfg,
            source_cd=src_cd,
    ...
    def extract_qacustomermonthly(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "extract_qacustomermonthly",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("Skip")
        ...
        v_cur_date = ti.xcom_pull(
            task_ids="source_file_stats", key="batch_timestamp")
        task_name = "extract_qacustomermonthly"
        run_extract_task(
            cfg=cfg,
            task_name=task_name,
            default_args=default_args,
