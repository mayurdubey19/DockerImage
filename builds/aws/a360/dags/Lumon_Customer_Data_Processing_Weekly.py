...
@dag(
    dag_id="Lumon_Customer_Data_Processing_Weekly",
    default_args=default_args,
    description="Lumon Customer_Data_Processing Data Pipeline",
    schedule_interval=None,
    max_active_runs=1,
    params={"run_time_params": {"task_to_skip": []}},
    access_control={
        "Lumon_admin": {"can_read", "can_edit", "can_delete"},
        "Lumon_ops": {"can_read", "can_edit", "can_delete"},
        "Lumon_a360user": {"can_read"}
    },
    catchup=False
)
def pipeline_function():
    ...
    if current_env == "Prod":
        json_config_path = default_args["config_path"] + \
            "/Lumon_Customer_Data_Processing_Weekly_prod.json"
    else:
        json_config_path = default_args["config_path"] + \
            "/Lumon_Customer_Data_Processing_Weekly.json"

    cfg = get_json_config(json_config_path)
    region_name = cfg["aws_reg_rs"]
    global_param_config = get_ssm_parameters(
        "a360_dev_global_param_config", region_name, True)
    ...
    redshift_iam_role = "arn:aws:iam::{}:role/{}".format(
        a360_account_id, cfg["redshift_iam_role"])
    emr_iam_role = "arn:aws:iam::{}:role/{}".format(
        a360_account_id, cfg["emr_iam_role"])
    ...
    def dps_mcd_hist_bkp(**kwargs):
        ...
        s3_key = f"{cfg['s3_key_athena_glue']}{cfg['dp_config']['weekly']['athena_partitioned_table']}/"
    ...
    def prepare_incoming_file_from_db(**kwargs):
        ...
        if not pg_result:
            ...
            for data in pg_result:
                ...
                incremental_field = data[column_names.index(
                    'incremental_field')]
    ...
    def source_file_stats(**kwargs):
        ...
        if env == "dev":
            ...
            # in case of empty df it is overwriting stg_source_file_stats_customer making file_record_count col to varchar
            if not df.empty:
                df.to_sql(
                    "stg_source_file_stats_customer",
                    con=engine,
                    if_exists="replace",
                    index=False,
                    schema="a360_stg",
                )
            # Get file_record_count for files with S3 select failure, capture in tmp table mzb_aet_stg.stg_source_file_stats_customer_tmp
            ...
            get_file_record_count(
                ...
                s3_bucket=bucket,
                table_source_file_stats="stg_source_file_stats_customer",
            ...
            procedure_call = "CALL a360_core.prc_check_dup_files_customer ({},'{}')".format(
    ...
    def stg_a360_medicare101as(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "stg_a360_medicare101as",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("SKip")
        ...
        if "MEDICARE101AS" in pulled_list:
            file_present_ind = True
        else:
            file_present_ind = False
        src_cd = "MEDICARE101AS"
        run_a360_stg_job(
            cfg=cfg,
            source_cd=src_cd,
    ...
    def stg_a360_qaprospectweekly(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "stg_a360_qaprospectweekly",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("SKip")
        ...
        if "QAPROSPECTWEEKLY" in pulled_list:
            file_present_ind = True
        else:
            file_present_ind = False
        src_cd = "QAPROSPECTWEEKLY"
        run_a360_stg_job(
            cfg=cfg,
            source_cd=src_cd,
            bucket_name=bucket_name,
    ...
    def stg_a360_qasuppressioncbweekly(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "stg_a360_qasuppressioncbweekly",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("SKip")
        ...
        if "QASUPPRESSIONCBWEEKLY" in pulled_list:
            file_present_ind = True
        else:
            file_present_ind = False
        src_cd = "QASUPPRESSIONCBWEEKLY"
        run_a360_stg_job(
            cfg=cfg,
            source_cd=src_cd,
            bucket_name=bucket_name,
    ...
    def mcd_matchai_file_receive(**kwargs):
        ...
        timestamp = task_instance.xcom_pull(
            task_ids="mcd_matchai_file_send", key="timestamp")
    ...
    def load_transactional_tables(**kwargs):
        ...
        procedure_call = f"""CALL a360_load.prc_a360_ods_txn_wrapper(
        {v_execid},
    ...
    def job_load_crc_prev(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            ...
            raise AirflowSkipException
        ti = kwargs["ti"]
        pulled_list = ti.xcom_pull(
            task_ids="source_file_stats", key="process_list_key")
    ...
    def prc_extract_reject_files(**kwargs):
        ...
        if len(pulled_list) > 0:
            ...
            logging.info(
                "Completed Reject file generation and transfer process")
        else:
    ...
    def extract_medicare101as(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "extract_medicare101as",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("Skip")
        ...
        v_cur_date = ti.xcom_pull(
            task_ids="source_file_stats", key="batch_timestamp")
        task_name = "extract_medicare101as"
        run_extract_task(
            cfg=cfg,
            task_name=task_name,
            default_args=default_args,
    ...
    def extract_qaprospectweekly(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "extract_qaprospectweekly",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("Skip")
        ...
        v_cur_date = ti.xcom_pull(
            task_ids="source_file_stats", key="batch_timestamp")
        task_name = "extract_qaprospectweekly"
        run_extract_task(
            cfg=cfg,
            task_name=task_name,
            default_args=default_args,
    ...
    def extract_qasuppressioncbweekly(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "extract_qasuppressioncbweekly",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("Skip")
        ...
        v_cur_date = ti.xcom_pull(
            task_ids="source_file_stats", key="batch_timestamp")
        task_name = "extract_qasuppressioncbweekly"
        run_extract_task(
            cfg=cfg,
            task_name=task_name,
            default_args=default_args,
    ...
    post_mcd_process_task = post_mcd_process()
    (dps_mcd_hist_bkp()
     >> business_to_internal_s3_file_transfer()
     >> prepare_incoming_file_from_db()
     >> source_file_stats()
     ...
     >> extract_qaprospectweekly()
     >> extract_qasuppressioncbweekly())
    post_mcd_process_task >> mcd_part2()
