...
default_args = {
    ...
    "retries": 0,
    "email_to": "Mitesh.Shivramwar@data-axle.com,harekrushna.mishra@data-axle.com",
...
@dag(
    dag_id="Lumon_Transaction_Data_Processing_Daily",
...
def pipeline_function():
    ...
    if current_env == "Prod":
        json_config_path = default_args["config_path"] + \
            "/Lumon_Transaction_Data_Processing_Daily_prod.json"
    else:
        json_config_path = default_args["config_path"] + \
            "/Lumon_Transaction_Data_Processing_Daily.json"
    ...
    def stg_a360_interactions(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "stg_a360_interactions",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("SKip")
        ...
        if "INTERACTIONS" in pulled_list:
    ...
    def stg_a360_lumoncustintr(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "stg_a360_lumoncustintr",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("SKip")
        ...
        if "LUMONCUSTINTR" in pulled_list:
            file_present_ind = True
        else:
            file_present_ind = False
        src_cd = "LUMONCUSTINTR"
        run_a360_stg_job(
            cfg=cfg,
            source_cd=src_cd,
            bucket_name=bucket_name,
    ...
    def mcd_part1_match_indicator(**kwargs):
        ...
        mcd_match_indicator(cfg, aet_account_id)
