...
default_args = {
    ...
    "retries": 0,
    "email_to": "Mitesh.Shivramwar@data-axle.com,harekrushna.mishra@data-axle.com,mayur.dubey@data-axle.com,shreyash.nigam@data-axle.com,pradeep.ingle@data-axle.com,shreya.sharma@data-axle.com",
    "email_subject": "Lumon Daily Processing Job: ",
...
@dag(
    dag_id="Lumon_Customer_Data_Processing",
    access_control={"Lumon": {'can_read','can_edit','can_delete'}},
    default_args=default_args,
    description="Lumon Customer Data Daily Data Pipeline",
...
def pipeline_function():
    ...
    if current_env == "Prod":
        json_config_path = default_args["config_path"] + \
            "/Lumon_Customer_Data_Processing_config_prod.json"
    else:
        json_config_path = default_args["config_path"] + \
            "/Lumon_Customer_Data_Processing_config.json"
    ...
    def dps_mcd_hist_bkp(**kwargs):
        ...
        #          values
        #          ('Lumon Daily Cycle','Lumon Daily Cycle Started  '||current_date,current_timestamp,18,'INGESTION','DEFAULT') 
        #        """
    ...
    def source_file_stats(**kwargs):
        ...
        if env == "dev":
            ...
            engine = get_redshift_sqlalchemy_conn(
                "us-east-2", cfg["secret_name"], cfg["redshift_db"], secrets_config
            )
            ...
            get_file_record_count(
                ...
                glue_job_name="glue_lumon_daily_source_file_stats",
    ...
    def dps_extract_mcd_incr_demo(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            ...
            print("SKip")
            raise AirflowSkipException
        del_obj = del_all_object_from_s3_folder(
            bucket_name, "mcd_input/")
    ...
    def mcd_candidate_unload(**kwargs):
        if kwargs["dag_run"].conf.get("run_time_params") and skip_task_check(
            "mcd_candidate_unload",
            kwargs["dag_run"].conf.get("run_time_params").get("task_to_skip"),
        ):
            # send_email()
            print("SKip")
        ...
        logging.info("Mcd candidate Unload Process")
        mcd_run_process(
            "candidate_unload.py",
            cfg,
            mdm_bucket_name,
    ...
    def mcd_correlate(**kwargs):
        ...
        logging.info("Mcd Correlation Process")
        mcd_run_process(
            "correlate.py",
            cfg,
            mdm_bucket_name,
