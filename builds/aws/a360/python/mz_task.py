...
def dp_process_setup(
    ...
    query_type = ""
    sql = (
        f"select * from geha_core.tmdm_config_sql where step_set = '{step_set}' "
...
def extract_reject_files(**kwargs):
    ...
    for src_cd in pulled_list:
        ...
        input_filename_col = f"filename_geha_dataaxle_{src_cd}"
        ...
        for index, row in df_input_list.iterrows():
            ...
            UNLOAD_SQL = f"""UNLOAD ('SELECT * FROM spectrum_geha_ods.{athena_reject_table} WHERE {input_filename_col} = ''{input_filename}''') 
        ...
        for index, row in df_input_list2.iterrows():
            ...
            update_sql = f"""UPDATE GEHA_CORE.TMDM_SOURCE_FILES
...
def get_processing_list(cfg, secrets_config, batchid):
    tuple_data = redshift_a360.redshift_query_execute(
        sql="""SELECT distinct t1.source_typ_code FROM geha_core.tmdm_source t1 JOIN geha_core.tmdm_source_files t2 
...
def generate_excel_report(
    ...
    **kwargs,
    ...
    if "sql_str" not in kwargs:
        v_sql = (
            "select * from geha_core.tmdm_config_sql "
...
def mcd_run_process(py_name, cfg, mdm_bucket_name, application_id, emr_iam_role):
    ...
    response = client.start_job_run(
        ...
        name="geha_{}".format(py_name.split(".")[0]),
    )
    ...
    while job_run_flag == "start":
        ...
        if job_run_status == "SUCCESS":
            logging.info("Geha Mcd Script {} Process Success ".format(py_name))
            break
        elif job_run_status in ("FAILED", "CANCELLING", "CANCELLED"):
            logging.info(
                "Geha Mcd Script {} Process Status: {} ".format(py_name, job_run_status)
            )
            raise Exception("Geha Mcd {} Script Failed, please review.".format(py_name))
...
def send_file_stats(step_set, email_subject, cfg, secrets_config):
    ...
    if current_env == "Prod":
        email_business_users_list = [
            "AETSupport@data-axle.com",
            "AetnaCampaignManagementExternal@data-axle.com",
        ]
    else:
        email_business_users_list = ["AETSupport@data-axle.com"]

    mail_body = get_html_format(query_step_sql, cfg, secrets_config)
    print("Step : {} Mail body {}".format(step_set, mail_body))
    email.send_email(
        client="Aetna",
        email_address="AETSupport@data-axle.com",
...
def get_file_record_count(
    ...
    **kwargs,
    ...
    submit_glue_job = GlueJobOperator(
        task_id="glue_source_file_stats",
        job_name="glue_source_file_stats",
...
def geha_gen_extract(
    ...
    sql = f"call geha_load.prc_geha_extracts_trkg({v_execid}, 'Outbound Extracts':: character varying, '{p_cur_user}'::character varying, 'R', '{p_freq}'::character varying, 'X'::character varying)"
    ...
    logging.info("prc_geha_extracts_trkg for 'R' Complete")

    record_set = redshift_a360.redshift_query_execute(
        sql=f"""select view_name FROM geha_ods.geha_outbound_extracts_hist where frequency = '{p_freq}' and exec_id = {v_execid} and """
        f"""process_status = 'R' """,
        cfg=cfg,
        secrets_config=secrets_config,
        query_type="query",
    ...
    for vw in view_names:
        v_unload_sql = f"""
                        unload ('select * from geha_dwh.{vw}')
        ...
        logging.info(" Geha dwh views Unload Complete")

        procedure_call = "CALL geha_load.prc_geha_extracts_trkg ({},'{}','{}','{}','{}','{}')".format(
...
def rename_outbound_extract(cfg, bucket_name, p_cur_date):
    ...
    s3_a360.s3_put_object(
        ...
        key=f"{destination_folder}/GEHA_OUTBOUND_EXTRACTS_CTL_{file_ts}.txt",
        body=trigger_data.encode(),
    )
    logging.info("Final Report file placed in S3.")

    s3_path = f"s3://{bucket_s3}/{destination_folder}/GEHA_OUTBOUND_EXTRACTS_CTL_{file_ts}.txt"
...
def da_source_files_stats(**kwargs):
    ...
    if count_update:
        ...
        print("geha_core.prc_check_dup_files proc loading starts....")
        procedure_call = "CALL geha_core.prc_check_dup_files ({},'{}')".format(
